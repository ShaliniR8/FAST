 ...
 *	Add a note
 */
		addNote: function(relx, rely, text, titles, icon) {
			var self = this;
			this.noteCount++;
			var elem = this.options.onAdd.call(this, text, icon);
			var $elem = $(elem);
			$(this.img).imgViewer("addElem",elem);
			$elem.data("relx", relx).data("rely", rely).data("note", text).data("titles", titles);
 ...
 */
		import: function(notes) {
			var self = this;
			$.each(notes, function() {
				self.addNote(this.x, this.y, this.note, this.titles, this.icon);
			});
			$(this.img).imgViewer("update");
		},
/*
