...
task :update_blank_time_zones => :environment do
  ...
  submissions = Submission.all.select { |s| s.event_time_zone.blank? }.each { |s| s.event_time_zone = CONFIG::GENERAL[:time_zone]}
  Submission.transaction do
    submissions.each(&:save!)
    puts "UPDATED SUBMISSIONS"
  end
  records = Record.all.select { |r| r.event_time_zone.blank? }.each { |r| r.event_time_zone = CONFIG::GENERAL[:time_zone]}
