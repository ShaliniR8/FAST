task :update_record_fields_airport => :environment do

	# Get submission fields where stores airport info
	record_fields = RecordField.where("fields_id in (select id from fields where label like '%airport%') and value<>'' and value is not null")

	# store icao and iata value


	record_fields.each do |x|
