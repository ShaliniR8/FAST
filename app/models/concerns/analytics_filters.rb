module AnalyticsFilters
  ...
  def can_be_accessed(current_user)
    ...
    if self.to_s == "Submission"
      ...
      viewer_access_templates = is_admin ? Template.all.map(&:id) : Template.where(name: current_user.get_all_templates_hash[:viewer_template_deid])
      reports = Report.joins(records: :template)
  ...
  def within_timerange(start_date, end_date)
    ...
    when 'CorrectiveAction', 'Audit', 'Inspection', 'Evaluation', 'Investigation', 'Finding', 'SmsAction', 'Recommendation', 'Sra', 'RiskControl' #Removed Hazard from here because that was causing inconsistencies in the mitigated risk matrix
      return where("#{table_name}.due_date >= ? && #{table_name}.due_date <= ?", start_date.utc.to_date, end_date.utc.to_date)
    else
