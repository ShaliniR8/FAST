class Audit < ActiveRecord::Base
  ...
  belongs_to  :created_by,        foreign_key: 'created_by_id',         class_name: 'User'
	...
	# 			FindingTransaction.create(:users_id=>session[:user_id],:action=>"Open",:owner_id=>f.id,:stamp=>Time.now)
	# 			#notify(f.responsible_user, Time.now+7.days, "Finding ##{f.get_id} has been scheduled for you." + "    <a href='#{finding_path(f)}'>#{'Click to view'}</a>", "Finding", f.id, "open")
	# 			f.status="Open"
