...
class SubmissionsController < ApplicationController
	...
	def index
		...
		@records = @records
			.filter_array_by_timerange(@records, params[:start_date], params[:end_date])
	...
	def create
		...
		if params[:anonymous] == "1"
			params[:submission][:anonymous] = true
		else
			params[:submission][:anonymous] = false
		end
			...
			if params[:commit] == "Submit"
				...
				if params[:create_copy] == "1"
					...
					convert_privileges = AccessControl.where(
						:action => 'notifier', 
						:entry => @record.template.map_template.name)
						.map{|x| x.privileges.map(&:id)}.flatten
					convert_notify_users = User.preload(:privileges)
						.where("disable is null or disable = 0")
						.keep_if{|x| 
							x.privileges.map(&:id) & convert_privileges != []}
				end
